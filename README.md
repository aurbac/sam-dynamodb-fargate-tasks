...
```bash
sam-dynamodb-fargate-tasks$ sam build --use-container
```
...
```bash
sam-dynamodb-fargate-tasks$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
sam-dynamodb-fargate-tasks$ sam logs -n HelloWorldFunction --stack-name sam-dynamodb-fargate-tasks --tail
```
...
```bash
sam-dynamodb-fargate-tasks$ pip install -r tests/requirements.txt --user
# unit test
sam-dynamodb-fargate-tasks$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-dynamodb-fargate-tasks$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam-dynamodb-fargate-tasks
```
